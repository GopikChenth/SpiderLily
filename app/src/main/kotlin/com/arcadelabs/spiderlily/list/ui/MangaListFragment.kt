...
import com.arcadelabs.spiderlily.alternatives.ui.AutoFixService
...
import com.arcadelabs.spiderlily.core.ui.list.FitHeightGridLayoutManager
import com.arcadelabs.spiderlily.core.ui.list.FitHeightLinearLayoutManager
	...
	override fun onItemClick(item: MangaListModel, view: View) {
		if (selectionController?.onItemClick(item.id) != true) {
			...
			if ((activity as? MangaListActivity)?.showPreview(manga) != true) {
				router.openDetails(manga, (activity as? MangaListActivity)?.title?.toString())
			}
		}
	}
